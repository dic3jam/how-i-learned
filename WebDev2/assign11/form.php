...
$people = array();
$xmlPath = "/assign11/xmls/";
$fullpath = "/var/sentora/hostdata/jmd06260/public_html/AdvWeb/";
...
function new_icecream($filename, $name, $flavor, $fullpath, $xmlPath) {
	...
	if(rename(( $fullpath . $filename), ($fullpath . $xmlPath . $filename)))
		return True;
	else
		return False;
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if($_POST["write"]){
		...
		$filepath = $fullpath . $xmlPath . $filename;

		if(file_exists($filepath))
			unlink($filepath);
			//write_icecream($filename, $flavor);
		if(new_icecream($filename, $name, $flavor, $fullpath, $xmlPath))
