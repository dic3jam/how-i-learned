...
if (!empty($_SESSION['cart'])) {
	...
	$q = "SELECT print_id, CONCAT_WS(' ', first_name, middle_name, last_name) AS artist, print_name FROM e_artists, e_prints WHERE e_artists.artist_id = e_prints.artist_id AND e_prints.print_id IN (";
	foreach ($_SESSION['cart'] as $pid => $value) {
		$q .= $pid . ',';
	}
	$q = substr($q, 0, -1) . ') ORDER BY e_artists.last_name ASC';
